        ...
        for (let i = 0; i < gamesData.length; i++) {
            ...
            for (let j = 0; j < gamesData[i].gamePlayers.length; j++) {
                ...
                if (gamesData[i].gamePlayers.length == 2) {
                    $('<td>' + gamesData[i].gamePlayers[j].player + '</td>').appendTo(row);
                }
                if (gamesData[i].gamePlayers.length == 1 && (data.player == "Guest" || data.player.id == gamesData[i].gamePlayers[j].id)) {
                    $('<td>' + gamesData[i].gamePlayers[0].player + '</td><td class="yellow500">WAITING FOR PLAYER</td>').appendTo(row);
                }
                if (gamesData[i].gamePlayers.length == 1 && data.player.id != null && data.player.id != gamesData[i].gamePlayers[j].id) {
                    $('<td>' + gamesData[i].gamePlayers[0].player + '</td><td class="yellow500">WAITING FOR PLAYER</td>').appendTo(row);
                ...
                if (gamesData[i].gamePlayers[j].id == data.player.id) {
...
function getPlayers(gamesData) {
        ...
        for (let i = 0; i < gamesData.length; i++) {
            ...
            for (let j = 0; j < gamesData[i].gamePlayers.length; j++) {
                ...
                if (!playersIds.includes(gamesData[i].gamePlayers[j].id)) {
                    playersIds.push(gamesData[i].gamePlayers[j].id);
                    let playerScoreData = {
                        "id": gamesData[i].gamePlayers[j].id,
                        "email": gamesData[i].gamePlayers[j].player,
                        "scores": [],
                        "total": 0.0
                    };
